...
build/libhdistjail.so.${SONAME}: build/hdistjail.o
	...
	${CC} -o $@ -Wl,-install_name,libhdistjail.so.${SONAME} $< ${LDFLAGS}
...
test: build/libhdistjail.so.$(SONAME)
	python test_jail.py --nocapture -v

simpletest: build/libhdistjail.so.$(SONAME)
	LD_PRELOAD=build/libhdistjail.so.$(SONAME) HDIST_JAIL_LOG=jail.log cat hello
